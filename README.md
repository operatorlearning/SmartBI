...
1. **为什么用 EasyExcel 而不是 POI？**
   → SAX 模式逐行读取，不加载全部数据到内存，百万行数据内存占用仅几十 MB

2. **AI 响应不稳定怎么办？**
   → Prompt 工程约束输出格式 + JSON Schema 校验 + 异常兜底保存原始响应

3. **异步任务如何保证可靠性？**
   → MQ 手动 ACK + 死信队列兜底 + 任务状态持久化

4. **接口限流用什么方案？**
   → Redis Lua 脚本实现滑动窗口计数器，原子操作保证并发安全

5. **SSE 和 WebSocket 区别？**
   → SSE 是单向推送、基于 HTTP、自动重连，适合进度通知场景；WebSocket 适合双向通信
